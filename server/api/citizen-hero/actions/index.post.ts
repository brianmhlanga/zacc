...
function validateActionFields(data: z.infer<typeof baseSchema>) {
  const needCta =
...
export default defineEventHandler(async (event) => {
  ...
  const data = baseSchema.parse(raw)
  try {
  ...
  const maxOrder = await prisma.citizenHeroAction.aggregate({
    where: { panelId: panel.id },
    _max: { sortOrder: true }
  })
  const sortOrder = (maxOrder._max.sortOrder ?? -1) + 1

  const created = await prisma.citizenHeroAction.create({
    data: {
      panelId: panel.id,
      sortOrder,
